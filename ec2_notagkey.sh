#!/bin/bash
#
# Title: ec2_notagkey.sh
#
# Description: ec2_nottagged.sh is used to create a list EC2 resource ID's (instances and
# volumes) that have no CostCenter tag key value and email the list
#
# Usage: ec2_notagkey.sh [AWS region]
#
# Requirements:
...
do
   # instances first...
   [[ -s /var/tmp/${Region}_instances_totag ]] && mail -s "${Region}: Instances without <tag_key> tag" your.email@domain.com < /var/tmp/${Region}_instances_totag
   rm /var/tmp/${Region}_instances_totag

   # volumes next...
   [[ -s /var/tmp/${Region}_volumes_totag ]] && mail -s "${Region}: Volumes without <tag_key> tag" bill.jorgensen@domain.com < /var/tmp/${Region}_volumes_totag
